...
namespace Gordon360.Services
{
    ...
    public class ScheduleService : IScheduleService
    {
        ...
        /// <param name="username">The AD Username of the user</param>
        /// <returns>CoursesBySessionViewModel if found, null if not found</returns>
        public async Task<IEnumerable<ScheduleViewModel>> GetSchedulesAsync(string username)
        {
            List<ScheduleCourseViewModel> courses = await _context.UserCourses.Where(x => x.Username == username).Select<UserCourses, ScheduleCourseViewModel>(c => c).ToListAsync();

            var coursesBySession = _context.CM_SESSION_MSTR.AsEnumerable()
                .GroupJoin(courses,
                           s => s.YRTRM_CDE_4,
                           c => c.YearTermCode,
                           (session, courses) => new ScheduleViewModel((SessionViewModel)session, courses ))
                .Where(cbs => cbs.Courses.Any());

            return coursesBySession.OrderByDescending(cbs => cbs.Session.SessionCode);
        }
    }
}