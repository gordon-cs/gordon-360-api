...
namespace Gordon360.Services
{
    public class LostAndFoundService(CCTContext context, IAccountService accountService) : ILostAndFoundService
    {
        ...
        public async Task<int> CreateMissingItemReport(MissingItemReportViewModel reportDetails, string username)
        {
            ...
            if (reportDetails.submitterUsername != "")
            {
                ...
                idNum = (await accountService.GetAccountByUsername(reportDetails.submitterUsername))?.GordonID;
            }
            else
            {
                idNum = (await accountService.GetAccountByUsername(username))?.GordonID;
            }
        ...
        public async Task<int> CreateActionTaken(int missingItemID, ActionsTakenViewModel ActionsTaken, string username)
        {
            // Get requesting user's ID number
            var idNum = (await accountService.GetAccountByUsername(username)).GordonID;
        ...
        public async Task<string> CreateFoundItem(FoundItemViewModel FoundItemDetails, string username)
        {
            if (!hasFullPermissions(username))
            {
                throw new ResourceNotFoundException();
            ...
            {
                idNum = (await accountService.GetAccountByUsername(FoundItemDetails.submitterUsername)).GordonID;
            }
            else
            {
                idNum = (await accountService.GetAccountByUsername(username)).GordonID;
            ...
            {
                finderID = (await accountService.GetAccountByUsername(FoundItemDetails.finderUsername)).GordonID;
            }
            ...
            {
                ownerID = (await accountService.GetAccountByUsername(FoundItemDetails.ownerUsername)).GordonID;
            }
        ...
        public async Task<int> CreateFoundActionTaken(string foundItemId, FoundActionsTakenViewModel FoundActionsTaken, string username)
        {
        ...
        public async Task UpdateFoundItemAsync(string itemID, FoundItemViewModel itemDetails, string username)
        {
            ...
            (int? guestFinderID, string? finderID) = await FoundDataHelper(itemDetails.finderFirstName,
            ...
            (int? guestOwnerID, string? ownerID) = await FoundDataHelper(itemDetails.ownerFirstName,
        ...
        /// </returns>
        /// <exception cref="ResourceNotFoundException">If the found item report with given id cannot be found in the database</exception>
        private async Task<(int?, string?)> FoundDataHelper(string? guestFirstName, 
                                             ...
                                             int? DBGuestID)
        {
            ...
            if (username != null)
            {
                newID = (await accountService.GetAccountByUsername(username)).GordonID;
            }
        ...
        {
            var original = await context.FoundItems.FindAsync(foundItemID) ?? throw new ResourceNotFoundException() { ExceptionMessage = "The Found Item Report was not found" };

            // Convert the requesting user's username (e.g., "first.last") to the unique ID using the account service.
            var requesterAccount = await accountService.GetAccountByUsername(username);
        ...
        /// </exception>
        public async Task<IEnumerable<FoundItemViewModel>> GetFoundItemsByOwner(string requestedUsername, string requestorUsername)
        {
            ...
            var requestedAccount = await accountService.GetAccountByUsername(requestedUsername);
