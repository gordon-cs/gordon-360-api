...
namespace Gordon360.Services
{
    public class ProfileService : IProfileService
    {
        ...
        {
            var query = _unitOfWork.ProfileRepository.GetByUsername(username);
            if (query == null)
            {
        ...
        {
            var query = _unitOfWork.ProfileRepository.GetAll();
            var result = query.Select<PROFILE_CUSTOM, ProfileCustomViewModel>(x => x);
            return result;
        }
        ...
        {
            var original = _unitOfWork.ProfileRepository.GetByUsername(username);
