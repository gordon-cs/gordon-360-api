...
public class ProfileService(CCTContext context, IConfiguration config, IAccountService accountService, webSQLContext webSQLContext) : IProfileService
{
    ...
    public IEnumerable<GetEmergencyContactViewModel> GetEmergencyContact(string username) 
        => context.EmergencyContact
            .Where(x => x.AD_Username == username)
            .Select(x => GetEmergencyContactViewModel.From(x));
