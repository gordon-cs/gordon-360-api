...
namespace Gordon360.Services
{
    public class HousingService : IHousingService
    {
        ...
        public int EditApplication(string editorId, string sess_cde, int apartAppId, string[] newApplicantIds, ApartmentChoiceViewModel[] newApartmentChoices)
        {
            ...
            // Get an array of apartment choices that are in the array recieved from the frontend but not yet in the database
            //IEnumerable<string> apartmentChoicesToAdd = newApartmentChoices.Except(oldApartmentChoices);
            var newApts = new HashSet<String>(newApartmentChoices.Select(apt => apt.HallName));
            var oldApts = new HashSet<String>(oldApartmentChoices.Select());
            var newApartmentChoicesToAdd = newApartmentChoices.Where(ApartmentChoiceSaveViewModel => !)

            // Get an array of apartment choices that in the database but not in the array recieved from the frontend
            //IEnumerable<string> apartmentChoicesToRemove = oldApartmentChoices.Except(newApartmentChoices);
            var apartmentChoicesToRemove = oldApartmentChoices.Where(=> !)
