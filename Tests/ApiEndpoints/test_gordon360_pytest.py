...
# 0.    Test_AllScheduleControlTest
# 1.    Test_AllMyScheduleTest
# 2.    Test_AllScheduleTest
# 3.    Test_AllAuthenticationTest
# 4.    Test_AllAccountTest
# 5.    Test_AllEmailTest
# 6.    Test_AllEventsTest
# 7.    Test_AllActivityTest
# 8.    Test_AllMembershipTest
# 9.    Test_AllMembershipRequestTest
# A.    Test_AllProfileTest
# B.    Test_AllDiningTest
# C.    Test_AllStudentEmploymentTest
# D.    Test_AllVictoryPromiseTest
# E.    Test_AdminTest
...
#    Expected Content -- schedule description of the currently logged in user.
    def test_schedulecontrol_put_description(self):
        self.session = self.createAuthorizedSession(username, password)
        self.url = hostURL + 'api/schedulecontrol/description/' + \
            put_description + '/'
...
#    Expected Content -- all custom events of the currently logged in user.
    def test_get_all_myschedule_objects_of_current_user(self):
        ...
        assert response.json()[0]["DESCRIPTION"] == put_description
        assert response.json()[0]["BEGIN_TIME"] == begintime
        assert response.json()[0]["END_TIME"] == endtime
...
#    with `eventId`  as a parameter
    def test_get_myschedule_objects_of_id(self):
        ...
        assert response.json()["DESCRIPTION"] == put_description
        assert response.json()["BEGIN_TIME"] == begintime
        assert response.json()["END_TIME"] == endtime
...
#    Expected Status code -- 200 Ok
#    Expected Content -- all schedule objects of the currently logged in user.
    @pytest.mark.skipif(not enrolledInPracticum, reason = \
...
#    Normal faculty works, student's don't work, facultytest doesn't work
    @pytest.mark.skipif(not unknownPrecondition, reason = \
...
#    Expected Content -- Json Object with access_token attribute.
    def test_authenticate_with_valid_credentials_as_student(self):
        ...
        self.token_payload = { 'username':username, 'password':password, \
            'grant_type':'password' }
...
#    Expected Content -- Json Object with access_token attribute.
    def test_authenticate_with_valid_credentials___activity_leader(self):
        self.session = requests.Session()
        self.url = hostURL + 'token'
        self.token_payload = { 'username':leader_username, \
            'password':leader_password, 'grant_type':'password' }
        response = api.post(self.session, self.url, self.token_payload)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
...
#    Expected Response Body -- profile of the email person
    def test_get_student_by_email(self):
        ...
        try:
            ...
            pytest.fail('Expected Json response body, got {0}.'\
                .format(response.text))
        assert response.json()["FirstName"] == activity_code_360
...
#    Expected Response Body -- profile info of {username}
    def test_get_search_by_username(self):
        ...
        assert response.json()["FirstName"] == activity_code_360
        assert response.json()["LastName"] == activity_code_360
...
#    Expected Response Body -- A json response with the student resource
    def test_get_list_of_emails(self):
        self.session = self.createAuthorizedSession(username, password)
        self.url = hostURL + 'api/emails/activity/' + activity_code_AJG + '/'
        response = api.get(self.session, self.url)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
                .format(response.status_code))
...
#    Expected Response Body -- Not Found error message
    @pytest.mark.skipif(not unknownPrecondition, reason = "Shouldn't allow access"\
        " because the activity id doesn't exist")
    def test_get_emails_for_activity_404(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = hostURL + 'api/emails/activity/thisIsATest'
        response = api.get(self.session, self.url)
...
#    Expected Status Code -- 404 Not Found
#    Expected Response Body -- Not Found error message
    @pytest.mark.skipif(not unknownPrecondition, reason = "Still returns" + \
        "200 OK regardless of term code")
    def test_get_all_chapel_events_fake_term(self):
        self.session = self.createAuthorizedSession(username, password)
        self.url = hostURL + 'api/events/chapel/thisIsAFakeTermCode/'
        response = api.get(self.session, self.url)
...
class Test_AllActivities(testCase):
# # # # # # # # # #
# ACTIVITY TESTS  #
# # # # # # # # # #
    ...
    def test_get_all_activities___Guest(self):
        ...
        if not response.status_code == 200:
            ...
            try:
                ...
                pytest.fail('Expected Json response body, got {0}.'\
                    .format(response.text))
        assert response.json()["ActivityCode"] == activity_code_360
        ...
        assert response.json()["ActivityBlurb"] == \
            activity_blurb_360
        assert response.json()["ActivityURL"] == activity_URL_360
        assert response.json()["ActivityType"] == activity_type_360
...
#    Expected Response Body -- Json object with activity resource
    def test_get_one_activity___activity_leader(self):
        ...
        assert response.json()["ActivityDescription"] == \
            activity_description_AJG
        assert response.json()["ActivityImagePath"] == \
            activity_image_path_AJG
        assert response.json()["ActivityBlurb"] == activity_blurb_AJG
        assert response.json()["ActivityURL"] == activity_URL_AJG
        assert response.json()["ActivityType"] == activity_type_AJG
        assert response.json()["ActivityTypeDescription"] == \
            activity_type_description_AJG
        assert response.json()["Privacy"] == True
        assert response.json()["ActivityJoinInfo"] == activity_join_info_AJG
...
#    Expected Response Body -- list of activities
    def test_get_activities_for_session___activity_leader(self):
        ...
        if not (type(response.json()) is list):
            pytest.fail('Expected list, got {0}.'.format(response.json()))
        assert response.json()[1]["ActivityCode"] == activity_code_AJG
        ...
        assert response.json()[1]["ActivityBlurb"] == \
            activity_blurb_AJG
        assert response.json()[1]["ActivityURL"] == activity_URL_AJG
        assert response.json()[1]["ActivityType"] == activity_type_AJG
...
#    Expected Response Body -- Updated activity information
    def test_update_activity___activity_leader(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = hostURL + 'api/activities/' + activity_code_AJG + '/'
        self.data = {
            "ACT_CDE" : activity_code_AJG,
            "ACT_BLURB" : activity_blurb_AJG,
            "ACT_URL" : activity_URL_AJG
        }
...
#    Expected Response Content -- A list of json Objects.
    def test_get_memberships_for_an_activity___activity_leader(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = hostURL + 'api/memberships/activity/' + activity_code_AJG + '/'
        response = api.get(self.session, self.url)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
                .format(response.status_code))
        ...
        if not (type(response.json()) is list):
            pytest.fail('Response was not a list.')
        assert response.json()[0]["ActivityCode"] == activity_code_AJG 
        if "IDNumber" in response.json()[0]:
            warnings.warn("Security fault, Gordon ID leak")
...
#    Expected Reponse Content -- A list of json objects
    def test_get_all_my_memberships___activity_leader(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = \
            hostURL + 'api/memberships/student/' + str(my_id_number) + '/'
        response = api.get(self.session, self.url)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
                .format(response.status_code))
        ...
        if not (type(response.json()) is list):
            pytest.fail('Response was not a list.')
        if my_id_number != response.json()[0]["IDNumber"]:
...
#    Expected Reponse Content -- A list of json objects
    def test_get_memberships_username___activity_leader(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = \
            hostURL + 'api/memberships/student/username/' + username + '/'
        response = api.get(self.session, self.url)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
                .format(response.status_code))
...
#    Expected Response Content --  A list of json objects.
    def test_get_all_memberships_for_someone_else___activity_leader(self):
        ...
        if not (type(response.json()) is list):
            pytest.fail('Response was not a list')
        if valid_id_number != response.json()[0]["IDNumber"]:
            warnings.warn("Security fault, not the user's ID")
    ...
    def test_post_new_membership_for_someone___activity_leader(self):
        ...
        for dic in req:
            ...
            if (reqID == self.membershipID):
                found = True      
                try:
                    assert dic['ActivityCode'] == activity_code_AJG
...
#    leader for AJG
    def test_not_get_membership_requests_for_activity(self):
        ...
        response = api.get(self.session, hostURL + 'api/memberships/student/' \
            + str(my_id_number))
        try:
            for membership in response.json():
                if(membership['ActivityCode'] == activity_code_AJG and \
                    membership['Participation'] in LEADERSHIP_POSITIONS):
...
#    session code 201510 does not work
    ...
    def test_post_valid_membership_request__as_member(self):
        ...
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        getResponse = api.get(self.session, hostURL + \
            'api/requests/activity/' + str(activity_code_AJG))       
...
#    requests for an activity.
    def test_get_membership_requests_for_activity(self):
        self.session = \
            self.createAuthorizedSession(leader_username, leader_password)
        self.url = hostURL + 'api/requests/activity/' + activity_code_AJG + '/'
        response = api.get(self.session, self.url)
        if not response.status_code == 200:
            pytest.fail('Expected 200 OK, got {0}.'\
                .format(response.status_code))
        ...
        if not (type(response.json()) is list):
            pytest.fail('Expected list in response body, got {0}.'\
                .format(response.json()))
        if activity_code_AJG != response.json()[0]["ActivityCode"]:
    ...
    def Test_Allow_someone_to_join_my_activity___activity_leader(self):
        ...
        try:
            ...
            if self.membershipID < 0: # membership creation was not successful
                ...
                api.delete(self.session, hostURL + 'api/memberships/' + \
...
#    Expected Response Body -- A json response with student employment info
    @pytest.mark.skipif(not cctService, reason = \
