...
- __The identity and data type of any parameters that must be passed from the frontend__
  - The parameter name is in the same place as the previous point, `[Route("someRoute/{X}")]` in the respective Housing Controller method
  - The data type, let's call it Y, is the type of the parameter in the actual method declaration: `public IHttpActionResult DoSomething(Y X)` 
  - If the data type is a View Model, look for it in the View Models folder and you will find that the method requies a collection of parameters with various data types; that means make sure you are sending it a JSON string with all the necessary parameters
...
- __The code that saves an application__
  ...
  - This covers saving almost everything: the applicants, their off-campus program, the application ID (which is autogenerated in the database), and the ranking of halls applied for
...
- __The code that is used to figure out whether to show admin or applicant view__
  - `public bool CheckHousingStaff(...)` in the Housing Service
